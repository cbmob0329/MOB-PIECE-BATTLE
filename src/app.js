...
import {validateDeck,autoBuildDeck,switchDeckSlot} from './game/deck.js';
...
import {historyScreen,hallOfFameScreen} from './screens/records.js';
import {
...
import {claimMission} from './game/missions.js';
import {applyBattleFigureRecords,enrichedHistoryRow,figureRecord} from './game/records.js';
...
let query='';let rarity='ALL';let collectionStatus='ALL';let displaySlot=0;let displayQuery='';
...
function competitionBattleRequest(type){
  ...
  if(type==='cup'){
    ...
    return {request:{mode:'competition',title:['QUARTER FINAL','SEMI FINAL','FINAL'][t.round]||'RANK UP TOURNAMENT',opponentName:opponent,cpuRank:rank,context:{type},forfeitCountsLoss:true},resolve:result=>{const after=recordRankUpResult(profile,result.won);normalizeBattleProgress(profile);recordHistory(result,{title:['QUARTER FINAL','SEMI FINAL','FINAL'][t.round]||'RANK UP TOURNAMENT',opponentName:opponent},{mode:'rankup-tournament',rank});let message=result.won?(after.status==='finished'?(after.rankAfter!==after.rankAtEntry?`RANK UP ${after.rankAtEntry} → ${after.rankAfter}`:'TOURNAMENT WINNER'):'次のラウンドへ進出'):'トーナメント敗退';return {reward:after.status==='finished'?after.reward:null,message};}};
  }
  if(type==='league'){
    const next=currentLeagueMatches(profile).find(x=>!x.result);if(!next)throw new Error('現在プレイできるMOBリーグ本戦がありません。');
    return {request:{mode:'competition',title:'MOB LEAGUE',opponentName:next.name,cpuRank:next.rank||'SS',context:{type},forfeitCountsLoss:true},resolve:result=>{recordLeagueFinalResult(profile,result.won,result.battleFor,result.battleAgainst);syncCompetition(profile);recordHistory(result,{title:'MOB LEAGUE',opponentName:next.name},{mode:'mob-league',rank:next.rank});return {message:`MOB LEAGUE ${result.won?'WIN':'LOSE'} · ${result.battleFor}-${result.battleAgainst}`};}};
  }
  if(type==='master'){
    ...
    return {request:{mode:'competition',title:'MOB MASTER MATCH',opponentName,cpuRank:row?.rank||'SS',context:{type},forfeitCountsLoss:true},resolve:result=>{const after=recordMasterChallengeResult(profile,result.won,result.battleFor,result.battleAgainst);syncCompetition(profile);normalizeBattleProgress(profile);recordHistory(result,{title:'MOB MASTER MATCH',opponentName},{mode:'mob-master',rank:'SS'});const wonMaster=after.winner==='PLAYER';return {reward:wonMaster?MASTER_PRIZE:null,message:after.winner?(wonMaster?'MOB MASTER 獲得！':'MOB MASTER決定戦 敗北'):`SERIES ${after.defenderWins} - ${after.challengerWins}`};}};
  }
...
function openFigureInfo(id){
  ...
  dlg.innerHTML=`<section><button class="figure-dex-close" aria-label="閉じる">×</button><div class="figure-dex-hero ${known?'':'locked'}">${art(f)}</div><small>${f.displayNo} · ${f.rarity} · COST ${f.mobPiece.cost}</small><h2>${known?f.name:'？？？'}</h2><p class="figure-dex-owned">所持 ${owned} / 出場 ${rec.appearances} / 対戦勝利 ${rec.matchWins}</p>${known?`<div class="figure-dex-stats"><span>HP<b>${f.mobPiece.hp}</b></span><span>ATK<b>${f.mobPiece.attack}</b></span><span>DEF<b>${f.mobPiece.defense}</b></span><span>SPD<b>${f.mobPiece.speed}</b></span></div><h3>TAG</h3><div class="figure-dex-tags">${names.length?names.map(x=>`<span>${x}</span>`).join(''):'<span>タグなし</span>'}</div><p class="figure-dex-note">ラウンド勝利 ${rec.roundWins} · このフィギュアを使った対戦勝利 ${rec.matchWins}</p>`:'<p class="notice">まだ所持していないフィギュアです。入手すると名前・能力・タグ・戦績が開放されます。</p>'}</section>`;
